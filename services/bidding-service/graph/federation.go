...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Bid":
		resolverName, err := entityResolverNameForBid(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Bid": %w`, err)
		}
		switch resolverName {
		...
		case "findBidByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findBidByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindBidByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Bid": %w`, err)
			}
...
func entityResolverNameForBid(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Bid", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Bid", ErrTypeNotFound))
			break
		}
		return "findBidByID", nil
	}
	return "", fmt.Errorf("%w for Bid due to %v", ErrTypeNotFound,
